...
public class Level3Tester_SimulationSIDIS {
    ...
    public static MultiDataSet getData(String dir,int max,double beamE){
        ...
        while (counter_tot < max && start < 56) {
            ...
            while (r.hasNext() && counter_tot < max) {
                ...
                for (int sect = 1; sect < 7; sect++) {
                    ...
                    for (Level3Particle part : particles) {
                        if (part.Cal_Sector == sect ){//&& nPart_pSect(particles, sect)==1) { //&& nTrack_pSect(dsts[1],sect)==1
                            if (part.TruthMatch(0.1, 0.1, 0.1)) {
                                if (part.MC_PID == 11) {//MC_PID
                                    if (part.P > 0.) {
                                        if (part.check_Energy_Dep_Cut() == true
                                                && part.check_FID_Cal_Clusters(dsts[4]) == true
                                                && part.check_SF_cut() == true 
                                                && part.Nphe>=2){
                                                //&& part.Cal_Sector==part.HTCC_Sector
                                                //&& part.Cal_Sector==part.Sector
                                                //&& noCloseMCPart(part, dsts[5])) {
                                            hasParticle = true;
                                            p = part.P;
                    ...
                    if (nodeDC.getRows() > 0 && nodeEC.getRows() > 0 && counter_tot<max ) {//&& hasParticle == true && classs == 1
    ...
    public void test(MultiDataSet data,double thresh,int elClass,int LabelVal, String Part,Boolean mask_nphe) {
        ...
        INDArray metrics=Level3Tester_SimulationSIDIS.getMetrics(outputs[0],data.getLabels()[0],LabelVal,thresh, elClass);
        System.out.printf("\n Threshold: %f\n", thresh);
    ...
    public static void debugBG(INDArray outputs, MultiDataSet data,int elLabelVal,int elClass){
        ...
        H1F hBGType = new H1F("BG PID IDed as signal", 7, -0.5, 6.5);
        hBGType.attr().setLineColor(2);
        hBGType.attr().setFillColor(2);
        hBGType.attr().setLineWidth(3);
        hBGType.attr().setTitleX("BG PID (0 empty, 1 -211, 2 -11, 3 22, 4 11, 5 2112, 6 others)");
        ...
        c2.draw(hHasHTCC).draw(hphHasHTCC,"same").draw(hothHasHTCC,"same").draw(hnHasHTCC,"same");
    ...
    public static void main(String[] args){        
        ...
        int elClass=5;//1 for 2 classes, 2 for 3 classes, 3 for 4 classes etc
        int elLabelVal=1;
        MultiDataSet data=Level3Tester_SimulationSIDIS.getData(dir,10000,10.6);//10000
